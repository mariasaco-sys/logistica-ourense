...
# --- 1. BASE DE DATOS DE COORDENADAS (RESPALDO) ---
COORDENADAS_FIJAS = {
...
# --- 2. DATOS HISTÓRICOS (GENERADOS DE FORMA SEGURA) ---
# Usamos una lista simple para evitar errores de comillas triples gigantes
DATOS_RAW = [
...
def cargar_historial():
    try:
        # Reconstruimos el DataFrame desde la lista segura
        header = "Ruta_Asignada,Código postal envío,Ciudad_Clean,Num_Pedidos_Historico,Dia_Asignado"
        csv_content = header + "\n" + "\n".join(DATOS_RAW)
        
        df = pd.read_csv(io.StringIO(csv_content), dtype={'Código postal envío': str})
